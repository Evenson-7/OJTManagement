...
function AdminDash() {
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case "supervisors":
        const filteredSups = getFilteredData(supervisors, true); // true = disable dept filtering logic for college
        return (
          <div className="space-y-6 animate-fadeIn">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredInterns.map((intern) => (
                      <tr key={intern.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-gray-600">
                          {intern.supervisorId ? <span className="text-blue-600 font-medium">{getSupervisorName(intern.supervisorId)}</span> : <span className="text-red-400 italic">Unassigned</span>}
                        </td>
                        <td className="px-6 py-4"><span className="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded-full">Active</span></td>
                      </tr>
                    ))}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${sidebarExpanded ? 'md:ml-64' : 'md:ml-16'} pb-20 md:pb-0`}>
        <TopNav user={user} onProfileClick={() => setShowProfile(true)} onLogoutClick={() => setShowLogoutConfirm(true)} isMobileMenuOpen={isMobileMenuOpen} setIsMobileMenuOpen={setIsMobileMenuOpen} sidebarExpanded={sidebarExpanded} />
