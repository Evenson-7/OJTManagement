...
function CorDash() {
  ...
  // Default filter: If coordinator has a dept ID, use it, else 'all'
  const [filterDept, setFilterDept] = useState("all");

  useEffect(() => {
    // Set initial filter based on user profile when it loads
  ...
  const getFilteredInterns = () => {
    return interns.filter(intern => {
      ...
      // 2. Filter by Search (Name or School)
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case "supervisors":
        // Filter supervisors only by search (they don't usually have college IDs)
      ...
      case "interns":
        ...
        return (
          <div className="space-y-6 animate-fadeIn">
            ...
            {filterDept !== 'all' && (
              <div className="mb-2">
                 <span className="text-xs font-semibold bg-blue-50 text-blue-600 px-2 py-1 rounded-md border border-blue-100">
                    Filtering by: {getCollegeName(filterDept)}
                 </span>
              </div>
            )}
                      ...
                      <tr key={intern.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-gray-600">
                          <span className="bg-gray-100 text-gray-600 text-[10px] px-2 py-0.5 rounded-full border border-gray-200">
                            {getCollegeName(intern.departmentId)}
                          </span>
                        </td>
                         <td className="px-6 py-4 text-gray-600">{intern.course || 'N/A'}</td>
                        <td className="px-6 py-4 text-blue-600 font-medium">{getSupervisorName(intern.supervisorId)}</td>
    ...
    <div className="min-h-screen bg-gray-50 flex font-sans">
      ...
      <div className={`hidden md:flex bg-white shadow-lg flex-col fixed h-full z-30 transition-all duration-300 ${sidebarExpanded ? 'w-64' : 'w-16'}`}>
        ...
        <div className="px-2 py-4 border-t border-gray-200 bg-gray-50">
          <button onClick={() => setShowProfile(true)} className={`w-full flex items-center px-3 py-3 rounded-lg hover:bg-white transition-colors ${!sidebarExpanded ? 'justify-center' : ''}`}>
          ...
          <button onClick={() => setShowLogoutConfirm(true)} className={`w-full flex items-center px-3 py-3 mt-2 rounded-lg text-red-600 hover:bg-red-50 transition-colors ${!sidebarExpanded ? 'justify-center' : ''}`}>
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${sidebarExpanded ? 'md:ml-64' : 'md:ml-16'} pb-20 md:pb-0`}>
        <TopNav user={user} onProfileClick={() => setShowProfile(true)} onLogoutClick={() => setShowLogoutConfirm(true)} isMobileMenuOpen={isMobileMenuOpen} setIsMobileMenuOpen={setIsMobileMenuOpen} sidebarExpanded={sidebarExpanded} showRole={false} />
        <main className="flex-1 overflow-auto bg-gray-50 mt-16 md:mt-20"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">{renderContent()}</div></main>
      </div>
